...
import { TOKEN_PROGRAM } from './consts';
import { Metadata } from '@/solana/config/metaplex';
import {
...
export type Options = {
  ...
  /**
   * UpdateAuthority public address
   */
  updateAuthority?: StringPublicKey;
  /**
   * Check metadata
   */
  isCheckMetadata?: boolean;
  /**
...
export const getParsedNftAccountsByOwner = async ({
  ...
  if (!isCheckMetadata) {
    return splAccounts
      ...
      .map((t) => {
        ...
        accountMap.set(address, t.pubkey.toString());
        return { mint: address, tokenAccount: t.pubkey.toString() };
      });
  }
  ...
  >[] = await Promise.allSettled(
    chunks(metadataAccounts, 99).map((chunk: PublicKey[]) =>
  ...
  const accountsFiltered1 = accountsDecodedMeta
    .filter(onlySuccessfullPromises)
  ...
  if (updateAuthority) {
    accountsFiltered = accountsFiltered1.filter((row) => {
      // @ts-ignore
      const { updateAuthority: nftUpdateAuthority } = row;
      try {
        if (
          !new PublicKey(updateAuthority).equals(
            new PublicKey(nftUpdateAuthority),
          )
        ) {
          return false;
        }
    ...
    accountsFiltered = accountsFiltered1;
  }

  const accountsLastFiltered = [];
  for (const row of accountsFiltered) {
    if (accountMap.has(row.mint)) {
      accountsLastFiltered.push({
        tokenAccount: accountMap.get(row.mint),
        ...row,
      });
    }
  }
